...
dat=zeros(12,0); count=0;
doanimation=(length(dbstack)==1); %figure out if we're called from the command line or not
...
for i=dt/2:dt:20,
    ...
    angf=(mod(edgeang(angles(:,2))-edgeang(angles(:,1))-anglesl+pi,2*pi)-pi).*anglessp-angv.*anglesf.*min(dist(angles')./edgel(angles'));
    ...
    p=p+v*dt;
	count=count+1; dat(:,count)=p(:);
    if doanimation, %animation code
...
%v is the velocity of each part of Beyonce; the first entry is the hip, 6th
%is the head; entries 2 through 5 are knees and feet

if ~doanimation %draw time-lapse version otherwise
    if (now-lastCalled)*60*60*24>0.25, %have we been called in the last quarter second?
        lastCalled=now;    
        dat=dat(:,1:5:end); n=size(dat,2); %select every 5th frame to draw
        figure(100);set(gca,'ColorOrder',hsv2rgb([(.5:n)'/n ones(n,1)*[.6 .8]]),'NextPlot','replacechildren'); %have to fight Matlab to get color
        plot(dat([6 1 2 3 2 1 4 5]*2-1,:),dat([6 1 2 3 2 1 4 5]*2,:),'',[-2,100],[0,0],'r',dat(11,:),dat(12,:),'ko','MarkerSize',10,'MarkerFaceColor','k');
        axis equal;axis([-2 10 -1 6]);
    end
end    